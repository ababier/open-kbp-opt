...
def make_dvh_metric_diff_plots(df_dvh_metrics: pd.DataFrame, constants: ModelParameters):
    ...
    for m in df_to_plot.index.unique():
        ...
        p_values = pd.DataFrame(
            plan_values.groupby(["Structure"]).apply(
                lambda x: mannwhitneyu(x.value, pred_values[pred_values["Structure"] == x.iloc[0, 1]].value, alternative=alternative_hyp)[1]
            )
        )  # [1] retrieves the p value from the mann-whitney u test
        ...
        ax = sns.boxplot(
            ...
            hue="Dose_type",
            linewidth=1,
            hue_order=[*constants.optimization_short_hands_dict.values()],
            order=structure_order,
            zorder=2,
            boxprops={"zorder": 2},
        )
        ax.set_ylim((number_of_structures - 0.5, -0.5))
        ax.fill_betweenx(y, limits[0], limits[1], where=np.round(y).__mod__(2) == 1, facecolor="grey", alpha=0.15, zorder=1)
...
def make_criteria_satisfaction_plots(names_to_rank: pd.Series, df_dvh_error_to_plot: pd.DataFrame, file_name_prefix: str, constants: ModelParameters):
    """
    ...
    ax = sns.pointplot(
        ...
        hue_order=[*constants.optimization_short_hands_dict.values()],
        join=False,
        order=rank_to_names.index.astype(str).to_list(),
        n_boot=1,
    )

    # Create grey bars for figure to separate prediction bins
    y = np.arange(-0.5, number_of_ranks - 0.5, 0.0001)
    ax.fill_between(y, 0, 100, where=np.round(y).__mod__(2) == 1, facecolor="grey", alpha=0.15)
    ...
    save_plot(constants, f"{file_name_prefix}-criteria", legend_cols=len(constants.optimization_short_hands_dict.values()))
    ...
    most_crit_sat = (
        satisfied_criteria.loc[satisfied_criteria.drop("Dose_type", axis=1).groupby("Prediction").idxmax().value].groupby("Dose_type").size()
    )
    print(
        f"The {file_name_prefix} criteria satisfaction ranged from " f"{satisfied_criteria.value.min():.3f} to {satisfied_criteria.value.max():.3f}"
    )
    print(
        f"The optimization models that satisfied the most {file_name_prefix.upper()} "
...
def make_opt_error(cs: ModelParameters, df_dose_error: pd.DataFrame, plotted_error: str, xlim: List or None = None):
    ...
    score_p_values = df_to_plot.T.apply(
        lambda x: wilcoxon(x, df_to_plot.T["Prediction"], alternative="greater")[1] if x.name != "Prediction" else 1, axis=0, result_type="expand"
    )
    ...
    ax = sns.boxplot(
        data=y_data, x="value", y="Dose_type", showfliers=False, linewidth=1, width=0.65, order=cs.optimization_short_hands_dict.values()
    )
...
def plot_weight_norms(df_objective_data: pd.DataFrame, cs: ModelParameters):
    ...
    sns.boxplot(
        data=norms_to_plot,
        x=weight_column_name,
        y=norm_name,
        showfliers=False,
        hue="Dose_type",
        linewidth=1,
        hue_order=list(cs.optimization_short_hands_dict.values())[1:],
...
def save_plot(cs: ModelParameters, dvh_error_label: str, legend_cols: [int, None] = None, ax: [plt.figure, None] = None):
    """
