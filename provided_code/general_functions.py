...
def load_file(file_name: str) -> dict:
    ...
    if file_extension == 'csv':
        ...
        if '/plan-weights/' in file_name:
            loaded_file = pd.read_csv(file_name, index_col=None,
                                      names=['Objective', 'Weight', 'Optimized objective', 'Input objective'])
...
def get_predictions_to_optimize(cs: ModelParameters,
                                set_names_to_omit: Tuple[str] = ('set_20', 'set_21')) -> (pd.Series, list):
    """
    Args:
