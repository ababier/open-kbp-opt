...
class PlanningModel:
    def __init__(
        self, patient: Patient, cs: ModelParameters, relative_or_absolute: str = "relative", mean_or_max: str = "max", inverse_plan: bool = False
    ) -> None:
        """
    ...
    def _set_variables(self) -> None:
        ...
        if self.mean_or_max == "max" and not self.inverse_plan:
            self.obj_variable = self._solver.NumVar(-pywraplp.Solver_infinity(), pywraplp.Solver_infinity(), "obj_variable")
    ...
    # Objectives
    def obj_norm(self, input_dose_obj: Union[float, int, np.ndarray], opt_dose_obj: pywraplp.Variable, objective_name: str) -> None:
        """
    ...
    def add_dummy_variable(self, lb: float = -pywraplp.Solver_infinity(), ub: float = pywraplp.Solver_infinity()) -> pywraplp.Variable:
        """
    ...
    def get_average_as_variable(self, array_to_average: dict, objective_name: str, mask_to_sum: np.ndarray = None) -> pywraplp.Variable:
        """
    ...
    def c_var_obj(self, roi: str, c_var_dict: dict, dose_multiple: Union[float, int, np.ndarray]):
        ...
        for c_var_level in c_var_dict:
            ...
            for roi_voxel, voxel in enumerate(roi_mask):
                c_var_opt_voxel_dose[roi_voxel] = self._solver.NumVar(0, pywraplp.Solver_infinity(), name=f"{roi}_{sign}_{c_var_level}_{voxel}")
    ...
    def add_spg_constraint(self, spg_limit: int = 65) -> None:
        ...
        beam_angles_set = np.unique(self.patient.beamlet_xyz.T[2])  # optimization beam angels
        ...
        for beamlet_num in range(self.patient.number_of_beamlets):
            spg_beamlet_differences[beamlet_num] = self._solver.NumVar(0, self.cs.max_beam, name=f"beamlet_difference_{beamlet_num}")
        ...
        for angle_idx, angle in enumerate(beam_angles_set):
            ...
            angle_beamlet_indices = np.argwhere(self.patient.beamlet_xyz.T[2] == angle).squeeze()
            angle_beamlet_coords = self.patient.beamlet_xyz[angle_beamlet_indices]  # coordinates on fluence map
            angle_beamlet_coords_rows = np.unique(angle_beamlet_coords.T[0])  # active rows in fluence map at current angle
            ...
            for row_idx, row in enumerate(angle_beamlet_coords_rows):
                ...
                row_beamlet_coords_columns = np.where(angle_beamlet_coords.T[0] == row)  # beamlet indices for active columns in row
                row_beamlet_indices = np.reshape(angle_beamlet_indices[row_beamlet_coords_columns], (-1,))
                column_positions_in_row = angle_beamlet_coords.T[1][row_beamlet_coords_columns]  # active column in fluence map along row
                ...
                for col_idx, col in enumerate(column_positions_in_row):
                    ...
                    if neighbouring_column_position_mask.size == 1:  # neighbouring beamlet is active
                        ...
                        self._solver.Add(spg_beamlet_differences[beamlet_full_idx] >= self.w[beamlet_full_idx] - self.w[beamlet_neighbour_full_idx])
                    else:  # no active neighbouring beamlet, so spg is equal to beamlet intensity
                        self._solver.Add(spg_beamlet_differences[beamlet_full_idx] >= self.w[beamlet_full_idx])
    ...
    def save_fluence_and_dose(self) -> None:
        ...
        dose_df = pd.DataFrame(
            data=sparse_dose["data"].squeeze(), index=sparse_dose["indices"].squeeze(), columns=["data"]  # dose values  # voxel indices
        )
        ...
        if self.inverse_plan:
            ...
            for c_idx, c_name in enumerate(self.constraint_names):
                objective_df.loc[c_name, "optimized objective"] = self._solver.LookupVariable(f"{c_name} obj").SolutionValue()  # get objective value
        else:
            objective_df = pd.DataFrame(index=self.constraint_names, columns=["weight", "optimized objective", "input objective"])
    ...
    def compare_dm_to_inverse_planning(self) -> None:
        ...
        # Get objective function weights
        objective_df = pd.DataFrame(index=self.constraint_names, columns=["weight", "optimized objective", "input objective"])
        for c_idx, c_name in enumerate(self.constraint_names):  # iterate through all constraints that evaluate objective differences
            # objective_df.loc[c_name, 'weight'] = self._solver.LookupConstraint(c_name).dual_value()  # get objective weight (as dual variable)
            objective_df.loc[c_name, "optimized objective"] = self._solver.LookupVariable(f"{c_name} obj").SolutionValue()  # get objective value
            # objective_df.loc[c_name, 'input objective'] = self.input_objective_values[c_idx]
