...
def save_score_summary(cs: ModelParameters, df_errors: pd.DataFrame, name: str, level=0):
    ...
    #
    # Test rank order
    spearman_test_results = rank.apply(
        lambda x: spearmanr(x, rank[f'Prediction'].values))
...
def make_criteria_satisfaction_df(cs: ModelParameters, df_clinical_criteria: pd.DataFrame,
                                  df_ref_clinical_criteria: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    reference_duplicates_df = pd.DataFrame(reference_duplicates, index=df_ref_clinical_criteria.index,
                                           columns=df_clinical_criteria.columns)
...
def make_criteria_satisfaction_summary(cs: ModelParameters, df: pd.DataFrame) -> [pd.DataFrame]:
    ...
    criteria_sat_summary = pd.concat((crit_summary.unstack(1),
    ...
    criteria_sat_summary = criteria_sat_summary.droplevel(0, axis=1)[
        ['Reference', *cs.optimization_short_hands_dict.keys(), 'Best']]
    ...
    criteria_sat_summary = criteria_sat_summary.apply(
        lambda x: x.replace(max_row_values[x.name], f'\textbf{{{max_row_values[x.name]}}}'),
        axis=1)
