...
class Patient:
    def __init__(self, cs: ModelParameters, identifier: str, patient_path: str, dose: np.array,
                 structure_masks: np.array, dij: sparse.csr_matrix,
                 voxel_dimensions: tuple, beamlet_indices: np.array, ring_sizes: tuple = (3, 6)) -> None:
        """
        ...
        self.beamlet_coords = beamlet_indices  # name change to convert to convention
    ...
    def _make_optimization_structs(self) -> None:
        """
        Generates the artificial optimization structures, which are designed to help with "dose shaping" in areas
        where there are no rois
        """
    ...
    def _initialize_patient_cKDTree(self) -> None:
        ...
        self.external_coords = np.array(np.unravel_index(external_indices, self.cs.patient_shape)).T
        # Convert coordinates to mm based on voxel size

        external_coords_mm = self.external_coords * self.voxel_dimensions
        # Make the cKD tree
        self.external_cKDTree = spatial.cKDTree(external_coords_mm)
    ...
    def make_rings(self) -> None:
        ...
        for struct in self.cs.rois['targets']:
            ...
            struct_coords = np.argwhere(self.structure_masks[:, :, :, struct_index])
            if struct_coords.size > 0:  # Only continue if the target exists
                struct_coords_mm = struct_coords * self.voxel_dimensions  # Convert xyz to spatial xyz values
                # Iterate through each ring size
                for ring_size in self.ring_sizes:
                    # Make the ring structure
                    ring_indices = self.make_ring_from_xyz(struct_coords_mm,
                                                           struct_coords,
                                                           ring_size=ring_size,
                                                           )
                    ...
                    self.opt_structures_attributes.loc[f'{struct}_{ring_size}mm_ring'] = [float(struct.strip('PTV')),
                                                                                          ring_size,
                                                                                          1]
    ...
    def make_lim_post_neck(self) -> None:
        ...
        roi_coords = np.argwhere(self.structure_masks[:, :, :, used_roi_index])
        ...
        if roi_coords.size > 0:
            ...
            # Convert to spatial xyx
            roi_coords_mm = roi_coords * self.voxel_dimensions

            # Make ring around roi
            ring_indices = self.make_ring_from_xyz(roi_coords_mm, roi_coords, ring_size=5)
            ring_coords = np.array(np.unravel_index(ring_indices, self.cs.patient_shape))

            # get bounds of ring
            max_coords = np.max(ring_coords, axis=-1)
            min_xyz = np.max(ring_coords, axis=-1)

            # Iterate through each slice of patients and make the limPostNeck
            lim_post_neck = []  # Initialize list to track voxels in limPostNeck
            for z_slice in np.unique(self.external_coords.T[-1]):  # Iterate through slices
                # Identify current slice of OAR
                if z_slice in ring_coords.T:
                    oar_slice = z_slice  # oar slice
                elif z_slice > max_coords[-1]:
                    oar_slice = max_coords[-1]  # Highest slice
                else:
                    oar_slice = min_xyz[-1]  # Lowest slice
                # Next check to make sure ring exists inside patient space
                if np.max(ring_coords[-1] == oar_slice):
                    # Identify points in ring using coordinates at oar_slice
                    max_x = np.max(ring_coords[1][ring_coords[-1] == oar_slice])  # highest x coordinate
                    min_x = np.min(ring_coords[1][ring_coords[-1] == oar_slice])  # lowest x coordinate
                    mean_y = np.mean(ring_coords[0][ring_coords[-1] == oar_slice])  # average y coordinate
                    # Get external slice
                    external_xy = self.external_coords[
                        self.external_coords.T[-1] == z_slice].T  # Get external contour at z_slice
                    # TODO: check whats indices vs coords
                    ...
                    lim_post_neck_mask = np.all((external_xy[1] > min_x,  # Above lowest x coordinate
                                                 external_xy[1] < max_x,  # Below highest x coordinate
                                                 external_xy[0] > mean_y),  # Above highest y coordinate
                                                axis=0)  # mask of lim post neck based on external_xy_indices
            ...
            self.opt_structures_attributes.loc['lim_post_neck'] = [0, 0, 0]
    ...
    def prune_optimization_structures(self) -> None:
        ...
        priority = self.opt_structures_attributes.sort_values(by=['ring', 'dose_level', 'expansion'],
    ...
    def _get_voxels_of_interest(self) -> None:
        """
        Use sampled voxels to generate new sampled tensors (dose, dij, and structure indices)
        based on the sampled voxels
        """
    ...
    def make_ring_from_xyz(self, ring_roi_mm, ringed_roi, ring_size=2):
        ...
        ring_indices = np.ravel_multi_index(self.external_coords[ring].T, self.cs.patient_shape)
    ...
    def update_weights(self):
        self.objective_df = load_file(self.get_weights_path())
