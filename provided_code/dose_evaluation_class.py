...
class EvaluateDose:
    ...
    def __init__(self, data_loader, dose_loader=None):
        ...
        self.new_criteria_metric_df = self.metric_difference_df.copy()
    ...
    def make_metrics(self):
        ...
        self.dose_score_vec = pd.Series(np.zeros(num_batches), index=self.data_loader.patient_id_list)
        ...
        if not self.data_loader.file_paths_list:
            ...
            for idx in tqdm.tqdm(range(num_batches)):
                ...
                if reference_dose is not None:
                    ...
                    self.reference_criteria_df = self.calculate_metrics(self.reference_criteria_df, reference_dose, merge_targets=True)
                ...
                if self.dose_loader is not None:
                    ...
                    self.new_criteria_metric_df = self.calculate_metrics(self.new_criteria_metric_df, new_dose, merge_targets=True)
                    # Evaluate mean absolute error of 3D dose
                    self.dose_score_vec[self.patient_list[0]] = np.sum(np.abs(reference_dose - new_dose)) / np.sum(self.possible_dose_mask)
